...
glyphs["o"] = ([top, right, bottom, left, topRight, bottomRight, bottomLeft, topLeft]) => {
  ...
  let strokeColor = getComputedStyle(document.querySelector('.tc-body')).color;
  ...
  let backgroundColor = getComputedStyle(document.querySelector('.tc-body')).getPropertyValue("background-color")
  inner.setAttribute("fill", backgroundColor);
...
function create(script) {
  let source = script.source || script.parseTreeNode.attributes.source;
  const zoom = Number(script.zoom || 0.3);
  const debug = script.grid;

  if(source == null || typeof source != 'string' ) source = "No source given";
  ...
  let width = 0;
  const height = script.height || diagram.length;
